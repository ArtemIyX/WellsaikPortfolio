...
describe('shared portfolio components', () => {
  it('renders UiButton defaults, attrs, pressed state, and loading protection', async () => {
    const onClick = vi.fn()
    const wrapper = mount(UiButton, { props: { onClick }, attrs: { name: 'action', 'data-test': 'button' }, slots: { default: 'Save' } })
    expect(wrapper.get('button').attributes()).toMatchObject({ type: 'button', name: 'action', 'data-test': 'button' })
  ...
  it('separates UiText visual role from its semantic element', () => {
    const wrapper = mount(UiText, { props: { as: 'h3', role: 'display', tone: 'accent' }, slots: { default: 'Heading' } })
  ...
  it('renders only supplied UiSection regions and wires its label', () => {
    const wrapper = mount(UiSection, { props: { labelledby: 'section-title' }, slots: { title: '<h2 id="section-title">Title</h2>', default: '<p>Content</p>' } })
  ...
  it('keeps UiBox a semantic, non-interactive container', () => {
    const wrapper = mount(UiBox, { props: { as: 'article', variant: 'accent' }, slots: { default: 'Content' } })
  ...
  it('selects RouterLink for to, native anchors for href, and rejects ambiguity', () => {
    const wrapper = mount(defineComponent({ components: { UiLink }, template: '<UiLink to="/work">Work</UiLink>' }), { global: { stubs: { RouterLink: { template: '<a class="router-stub"><slot /></a>' } } } })
    expect(wrapper.get('.router-stub').text()).toBe('Work')
    expect(mount(UiLink, { props: { href: 'https://example.com', external: true, newTab: true }, slots: { default: 'External' } }).get('a').attributes()).toMatchObject({ target: '_blank', rel: 'noopener noreferrer' })
    const invalid = mount(UiLink, { props: { to: '/', href: '/also-invalid' } as never, slots: { default: 'Invalid' } })
    expect(invalid.get('a').attributes('href')).toBeUndefined()
  })
  it('implements ThemeSelector v-model for both presentations', async () => {
    const wrapper = mount(ThemeSelector, { props: { modelValue: 'system', presentation: 'segmented' } })
